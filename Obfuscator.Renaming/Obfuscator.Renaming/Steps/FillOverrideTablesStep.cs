    ...
    public class FillOverrideTables : BaseStep
    {
        ...
        protected override void EndProcess()
        {
            foreach (var key in _methodOverrides)
            {
                ...
                {
                    if (method.Module != implementation.Module)
                        Debug.Assert(false);
        ...
        {
            //var reference = _resolver.ReferenceMethod(self, declaringType);         
        ...
        private  void AddBaseMethodCall(MethodDefinition method, MethodReference baseMethod)
        {
            ...
            //MethodDefinition @base = null;
            //var baseType = Helper.GetBaseTypeDefinition(type);
            //while (baseType != null)
            //{
            //    @base = baseType.Methods.SingleOrDefault(m => Helper.IsExplicitImplementation(method, m) || Helper.HaveSameSignature(m, method));
            //    if (@base != null)
            //        break;
            //    baseType = Helper.GetBaseTypeDefinition(baseType);
            //}
